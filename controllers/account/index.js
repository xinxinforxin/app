...
var modelOauth = require('../../models/oauth');
...
module.exports = function(router){
   ...
   router.post('/login', function(req, res, next) {
       ...
       client.post_form('/user/login', {
           ...
           if(result.code == -112 || result.code == -111 || result.code == -113){
               return res.send(result);
           } 

           logger.trace("---code----"+result.code);
           oauth.grant()(req,res,next);
       })
   });
   //国宝计划获取用户信息
  router.post("/guobao",oauth.authorise(),function(req,res){
      var accessToken = req.body.accessToken;
      modelOauth.getAccessToken(accessToken,function(data){
        console.log(data);
        res.send(data);
      })
  });
